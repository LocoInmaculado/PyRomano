...
def cli():
    ...
    roman_parser = subparsers.add_parser("roman-to-decimal", help="Convierte número romano a decimal")
    roman_parser.add_argument("roman", type=str, help="Número romano (ej. XII·)")

    # Conversión de decimal a romano
    decimal_parser = subparsers.add_parser("decimal-to-roman", help="Convierte decimal a número romano")
    decimal_parser.add_argument("decimal", type=float, help="Número decimal (ej. 12.25)")

    # Conversión de medidas
    measure_parser = subparsers.add_parser("convert-measure", help="Convierte medidas romanas")
    ...
    try:
        if args.command == "roman-to-decimal":
            print(NumRom.a_decimal(args.roman))
        elif args.command == "decimal-to-roman":
            print(NumRom.a_romano(args.decimal))
        elif args.command == "convert-measure":
